...
class AliceTypesHeadersTest < Minitest::Test
  # --- Defaults and Initialization ---

  def test_defaults_include_content_type_and_accept
    headers = Alice::Types::Headers.new
  ...
  def test_merges_given_headers_with_defaults
    input = { 'X-Custom' => 'foo' }
    headers = Alice::Types::Headers.new(input)
  ...
  def test_reads_existing_header
    headers = Alice::Types::Headers.new('X-Test' => 'value')
  ...
  def test_to_h_returns_a_copy
    headers = Alice::Types::Headers.new('X-Test' => 'foo')
    copy = headers.to_h
    copy['X-Test'] = 'bar'
  ...
  def test_raises_when_headers_is_not_a_hash
    error = assert_raises(Alice::Error::ArgumentError) do
      Alice::Types::Headers.new('not a hash')
    end
