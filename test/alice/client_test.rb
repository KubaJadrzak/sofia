...
module Alice
  class ClientTest < Minitest::Test
    def setup
      @client = Alice::Client.new(base_url: 'https://httpbin.org', adapter: nil)
    end
    ...
    def test_performs_post_request_and_returns_response
      ...
      assert_equal 200, response.status
      assert_equal true, response.success?
      assert_equal({ 'name' => 'Alice' }, response.body['json'])
      assert_equal 'application/json', response.headers['content-type']
    end
    ...
    def test_performs_put_request_and_returns_response
      ...
      assert_equal 200, response.status
      assert_equal true, response.success?
      assert_equal({ 'name' => 'Updated Alice' }, response.body.to_h['json'])
    end

    def test_performs_patch_request_and_returns_response
      response = @client.patch do |req|
        req.path = '/patch'
        req.body = { name: 'Patched Alice' }
        req.headers['Content-Type'] = 'application/json'
      end

      assert_equal 200, response.status
      assert_equal true, response.success?
      assert_equal({ 'name' => 'Patched Alice' }, response.body.to_h['json'])
    end
